                ...
                for (var j = 1; j < tableBody.length; j++) {
                    ...
                    for (var k = 0; k < tableBody[j].length; k++) {
                        if (k === tableBody[j].length - 1) {
                            ...
                            var re = new RegExp("[]", "g");
                            html.push('<td  style="text-align:center; class="center overOut">' + tableBody[j][k].replace("[]", g_extraParam).replace("[]", g_extraParam).replace("[]", g_extraParam).replace("[]", g_extraParam).replace("[]", g_extraParam).replace("[]", g_extraParam).replace("[]", g_extraParam).replace("[]", g_extraParam).replace("[]", g_extraParam) + '</td>');
                        } else {
       ...
       // Tip = 6
        for (var i = 0; i < obj.length; i++) {
            if (obj[i].type === 1) {
                ...
                extra_bt_html.push(' <a id="' + obj[i].id+ '" href="' + obj[i].value + '"  class="btn btn-danger">' + obj[i].lable + '<i class="fa"></i></a>');
